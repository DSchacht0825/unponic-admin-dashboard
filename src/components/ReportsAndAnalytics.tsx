...
const ReportsAndAnalytics: React.FC = () => {
  ...
  const loadServiceDistribution = async (encounters: any[]) => {
    try {
      const { startDate, endDate } = getDateRange();
      console.log(`🎯 Service Distribution: Processing ${encounters.length} total encounters for range ${startDate} to ${endDate}`);
    ...
    const filteredEncounters = encounters.filter(encounter => {
      ...
      if (!encounterDate) {
        console.log('⚠️ No date field found for service distribution encounter');
        return false;
      }
      ...
      try {
        ...
        console.warn('⚠️ Invalid date format in service distribution:', encounterDate);
        return false;
      }
    });
    
    console.log(`🎯 Processing ${filteredEncounters.length} encounters for service distribution after date filtering`);
  ...
  const loadUserProductivity = async (encounters: any[]) => {
    try {
      const { startDate, endDate } = getDateRange();
      console.log(`👷 User Productivity: Processing ${encounters.length} encounters for range ${startDate} to ${endDate}`);
      
      // Filter encounters based on the selected date range
      const filteredEncounters = encounters.filter(encounter => {
        const encounterDate = encounter.interaction_date || encounter.date || encounter.created_at || encounter.encounter_date || encounter.timestamp;
        
        if (!encounterDate) {
          console.log('⚠️ No date field found for user productivity encounter');
          return false;
        }
        ...
        try {
          ...
          console.warn('⚠️ Invalid date format in user productivity:', encounterDate);
          return false;
        }
      });
  ...
  const loadMonthlyComparison = async (encounters: any[]) => {
    try {
      const { startDate, endDate } = getDateRange();
      console.log(`📅 Processing monthly comparison for ${encounters.length} encounters for range ${startDate} to ${endDate}`);
    ...
    // Determine how many months to show based on the selected time range
    const timeRangeDays = parseInt(timeRange);
    const monthsToShow = timeRangeDays <= 30 ? 2 : 
                       timeRangeDays <= 90 ? 3 : 
                       timeRangeDays <= 180 ? 6 : 12;
    ...
    const comparison = Object.entries(monthlyStats)
      ...
      .slice(-monthsToShow) // Dynamic number of months based on time range
      .map(([month, stats]) => ({
